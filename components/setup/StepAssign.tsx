    ...
    <div>
      ...
      <div className="bg-white border border-[#e8e6e0] rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <div className="text-sm text-gray-400">{role.slots} slot{role.slots !== 1 ? 's' : ''} needed each Sunday</div>
          </div>
          <span className="bg-[#eff6ff] border border-[#bfdbfe] text-[#2563eb] text-sm px-3 py-1 rounded-full font-medium">
        ...
        {/* Backup cascade */}
        <div>
          <div className="text-sm font-medium text-gray-700 mb-1 uppercase tracking-wide">
            Backup cascade
          </div>
          ...
          {assignment.backups.length === 0 ? (
            <p className="text-sm text-gray-400 italic mb-3">No backups added yet.</p>
          ) : (
            <div className="flex flex-col gap-2 mb-3">
              {assignment.backups.map((b, i) => (
                <div key={i} className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-[#eff6ff] border border-[#bfdbfe] rounded-full flex items-center justify-center text-xs font-medium text-[#2563eb] flex-shrink-0">
                    B{i + 1}
                  </div>
                  <select
                    value={b.volunteerId}
                    onChange={e => setBackup(role.id, i, e.target.value)}
                    className="flex-1 px-3 py-2.5 border border-[#e8e6e0] rounded-lg text-base bg-white focus:outline-none focus:ring-2 focus:ring-[#2563eb] focus:border-transparent"
                  >
          ...
          <button
            ...
            className="w-full border border-dashed border-[#e8e6e0] hover:border-[#2563eb] hover:text-[#2563eb] text-gray-400 text-sm py-2.5 rounded-lg transition-colors"
          >
            + Add backup
          </button>
        </div>
      ...
      <div className="flex justify-between items-center mb-8">
        <button
          ...
          className="text-base text-gray-400 hover:text-gray-700 disabled:opacity-30 transition-colors"
        >
          ← Previous role
        </button>
        <div className="flex gap-1.5">
          {roles.map((_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full transition-colors ${i === currentRole ? 'bg-[#2563eb]' : 'bg-[#e8e6e0]'}`} />
          ))}
        </div>
        <button
      ...
      <div className="flex justify-between">
        ...
        <button onClick={onNext} className="bg-[#2563eb] hover:bg-[#1d4ed8] text-white font-medium py-3 px-8 rounded-lg text-base transition-colors">
          Continue →
        </button>
      </div>
    </div>
